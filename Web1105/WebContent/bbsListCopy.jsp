...
<head>
<meta charset="UTF-8">
 <title>bbsListCopy.jsp복사본</title>
 <style type="text/css">
   *{font-size:16pt;}
...
//System.out.println(display);
 ...
 String b = "select rownum rn,sabun,name,title,wdate,pay,hit,email, (select count(*) from bbsreply r where r.sabun=b.sabun ) as rcnt  from ";
 ...
 msg = a+b+c+d;
  ...
  while(RS.next()==true) {
     ...
     Gwdate = RS.getTimestamp("wdate"); //추천
...
%>    	
  <tr>
    ...
    <td>  <%= Gsabun %> </td>
    <td>  <%= Gname %> </td>
    <td> <a href="bbsDetail.jsp?idx=<%=Gsabun%>"><%= Gtitle %><% if(Rcnt>0){%><span style='color:red;font-size:12pt'>[<%=Rcnt%>]</span><%}%></a> </td>
    <td>  <%= display %> </td>
    <td>  <%= Ghit %> </td>
    <td>  <%= Gemail %> </td>
  </tr>  
...
%>	
 ...
 <tr align="center">
 	<td colspan="7">
 	  <form name="myform" action="bbsListCopy.jsp">
 	    검색:
